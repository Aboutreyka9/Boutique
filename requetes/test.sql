...
LEFT JOIN versement ver ON v.code_vente = ver.transaction_code AND ver.type_versement = 'vente' WHERE  ver.etat_versement = 1 AND v.statut_vente  IN('validé','encaissé')
GROUP BY v.ID_vente
...
LEFT JOIN versement ver ON a.code_achat = ver.transaction_code AND ver.type_versement = 'achat'
 WHERE  ver.etat_versement = 1 AND a.statut_achat IN('validé','encaissé')
GROUP BY a.ID_achat;